...
#pragma omp parallel for
	...
	map<int, vector<int>> cliqueMembers; 
    map<int, float> correspondenceWeights; 

    // Step 1: Calculate the members of each clique and the weight of each correspondence
    for (int i = 0; i < igraph_vector_ptr_size(cliques); ++i) {
        ...
        for (int j = 0; j < igraph_vector_size(clique); ++j) {
            int nodeIndex = VECTOR(*clique)[j];
            cliqueMembers[i].push_back(nodeIndex);
            float nodeWeight = 0.0;
            for (int k = 0; k < igraph_vector_size(clique); ++k) {
                if (j != k) {
                    int neighborIndex = VECTOR(*clique)[k];
                    nodeWeight += Graph(nodeIndex, neighborIndex);
                }
            }
            correspondenceWeights[nodeIndex] = nodeWeight;
        }
    }
	//mutual voting
	for (auto& member : cliqueMembers) {
        for (int nodeId : member.second) {
            // Node to adjacent nodes voting
            float voteWeight = 0.0;
            for (int adjNodeId : member.second) {
                if (nodeId != adjNodeId) {
                    voteWeight += Graph(nodeId, adjNodeId);
                }
            }
            // Update the weight with the sum of votes
            correspondenceWeights[nodeId] += voteWeight;
        }
    }

    // Step 2: Find the clique with the highest weight
    int maxWeightCliqueIndex = -1;
    float maxCliqueWeight = 0.0;
    for (const auto& clique : cliqueMembers) {
        float cliqueWeight = 0.0;
        for (int member : clique.second) {
            cliqueWeight += correspondenceWeights[member];
        }
        if (cliqueWeight > maxCliqueWeight) {
            maxCliqueWeight = cliqueWeight;
            maxWeightCliqueIndex = clique.first;
        }
    }

    // Step 3: Find the correspondence with the lowest weight in the clique with the highest weight
    int minWeightCorrIndex = -1;
    float minWeight = numeric_limits<float>::max();
    for (int member : cliqueMembers[maxWeightCliqueIndex]) {
        if (correspondenceWeights[member] < minWeight) {
            minWeight = correspondenceWeights[member];
            minWeightCorrIndex = member;
        }
    }

    // Step 4: Find the correspondence with the highest weight among other cliques for exchange
    int maxWeightCorrIndex = -1;
    float maxWeight = 0.0;
    for (const auto& item : correspondenceWeights) {
        if (find(cliqueMembers[maxWeightCliqueIndex].begin(), cliqueMembers[maxWeightCliqueIndex].end(), item.first) == cliqueMembers[maxWeightCliqueIndex].end() && // 确保不在最大clique中
            item.second > maxWeight) {
            maxWeight = item.second;
            maxWeightCorrIndex = item.first;
        }
    }

    // Step 5: swap
    if (maxWeightCorrIndex != -1 && minWeightCorrIndex != -1) {
        swap(correspondence[minWeightCorrIndex], correspondence[maxWeightCorrIndex]);
        cout << "Exchanged correspondence " << minWeightCorrIndex << " with " << maxWeightCorrIndex << endl;
    }
